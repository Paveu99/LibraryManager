...
import { usePostBookMutation } from "../../../queries/books/usePostBookMutation";
import { BookDto } from "../../../../../library-manager-back/types";
import { useEffect, useState } from "react";
import { Button, TextField } from "@mui/material";
import styles from './styles.module.scss'

export const AddBook = () => {
    const { data, error, isPending, isSuccess, mutate } = usePostBookMutation()
    const {
        register,
        handleSubmit,
    ...
    const onSubmit = (payload: BookDto) => {
        mutate({
            ...payload,
            total_copies: payload.available_copies
        });
    };
    ...
    return <form onSubmit={handleSubmit(onSubmit)}>
        ...
        {error && (
            <p className={styles.error}>{error?.message || "Adding failed. Please try again."}</p>
        )}
